...
function getActorCount(actorType) {
    var http = new XMLHttpRequest();
    http.onreadystatechange = function () {
        if (http.readyState === 4) {
            end = new Date().getTime();
            if (http.status < 400) {
                returnData = JSON.parse(http.responseText);
                if (returnData) {
                    if (actorType === 'netcore') {
                        countDisplay2.innerHTML = returnData.count;
                    }
                    else {
                        countDisplay1.innerHTML = returnData.count;
                    }
    ...
    start = new Date().getTime();
    http.open("GET", "/api/ActorBackendService/?actorType=" + actorType + "&c=" + start);
    http.send();
}

/* This function calls the ActorBackendController's HTTP POST method to create a new actor in the ActorBackendService / ActorBackendServiceNetCore */
function newActor(actorType) {
    var http = new XMLHttpRequest();
    http.onreadystatechange = function () {
        if (http.readyState === 4) {
            end = new Date().getTime();
    ...
    start = new Date().getTime();
    http.open("POST", "/api/ActorBackendService/?actorType=" + actorType + "&c=" + start);
    http.send();
}
