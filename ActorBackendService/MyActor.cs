...
namespace ActorBackendService
{
    ...
    internal class MyActor : Actor, IMyActor, IRemindable
    {
        ...
        public async Task StartProcessingAsync(CancellationToken cancellationToken)
        {
            ...
            bool added = await StateManager.TryAddStateAsync<long>(StateName, 0);
        ...
        public async Task ReceiveReminderAsync(string reminderName, byte[] context, TimeSpan dueTime, TimeSpan period)
        {
            ...
            {
                long currentValue = await StateManager.GetStateAsync<long>(StateName);

                ActorEventSource.Current.ActorMessage(this, $"Processing actorID: {Id}. Current value: {currentValue}");

                await StateManager.SetStateAsync<long>(StateName, ++currentValue);

                var newValue = await StateManager.GetStateAsync<long>(StateName);
